...
from applications.compat import smart_text
...
class GFKQuerySet(QuerySet):
    ...
    def fetch_generic_relations(self, *args):
        ...
        if not app_settings.FETCH_RELATIONS:
            return qs
        ...
        if app_settings.USE_PREFETCH and hasattr(self, 'prefetch_related'):
        ...
        for ct_id, items_ in ct_map.items():
            if ct_id:
                ...
                objects = model_class._default_manager.select_related(
                    depth=app_settings.GFK_FETCH_DEPTH)
