...
from notifications import notify
from notifications.views import new_notification
...
class RelationshipStatus(models.Model):
    ...
    from_slug = models.CharField(_('from slug'), max_length=100, unique = False, help_text=_("Denote the relationship from the user, i.e. 'following'"))
    to_slug = models.CharField(_('to slug'), max_length=100, unique = False,  help_text=_("Denote the relationship to the user, i.e. 'followers'"))
    ####################
    from_role = models.ForeignKey(Role,related_name='from_role', verbose_name=_('from role'), null=True, blank=True)
    to_role = models.ForeignKey(Role,related_name='to_role', verbose_name=_('to role'), null=True, blank=True)
    #relationship_group_slug = models.CharField(_('relationship group slug'), max_length=100,
     #   help_text=_("Used to declare similar relationships, to be retrieved with a single call"))
    ####################
...
class RelationshipManager(User._default_manager.__class__):
    ...
    def canexist(self, user, status=None, symmetrical=False):
        """
        Returns boolean whether or not a relationship can exist between the given
        users.  An optional :class:`RelationshipStatus` instance can be specified.
        """
	if not status.from_role :
		if not status.to_role:
			return True
		else:
			if user.profile.role.id == status.to_role.id:
				return True
	else:
		if self.instance.profile.role.id == status.from_role.id:
			if not status.to_role.id:
				return True
			else:
				if user.profile.role.id == status.to_role.id:
					return True
    ...
    def has(self, status=None, symmetrical=False):
        """
        Returns boolean whether or not a user can have a specific relationship
        """
	if not status.from_role :
		if not status.to_role:
			return True
		elif self.instance.profile.role.id == status.to_role.id:
			return True
		else:
			return False
	else:
		if not status.to_role.id:
			return True
		elif self.instance.profile.role.id == status.from_role.id: 
			return True
		elif self.instance.profile.role.id == status.to_role.id:
			return True
        	else: 
    ...
    def can_be_followed(self, status=None, symmetrical=False):
        """
        Returns boolean whether or not a user can be followed according to a relationship status
        """
	if not status.from_role :
		if not status.to_role:
			return True
    ...
    def can_follow(self, status=None, symmetrical=False):
        """
        Returns boolean whether or not a user can follow others according to a relationship status
        """
	if not status.to_role :
		if not status.from_role:
			return True
